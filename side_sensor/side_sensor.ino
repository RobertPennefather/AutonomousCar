...
void loop() {
    ...
    //Just started around corner
    if ((distanceF < corner_threshold) && (distanceL > corner_threshold)){
      VR = vel_high;
      VL = vel_low;
    }

    //Can't detect anything close
    else if ((distanceF > corner_threshold) && (distanceL > corner_threshold)){
      VR = vel_low;
      VL = vel_low;
    }

    //Turn left, only front sensor can detect
    else if ((distanceL < corner_threshold) && (distanceF > corner_threshold)){
      VR = vel_low;
      VL = vel_med;
    }

    //Can detect wall with both
    else{
  
      //Go towards middle
      if(distanceL < dist_min){
        VR = vel_low;
        VL = vel_high;
      }
